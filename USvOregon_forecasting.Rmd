...
- **Predictors**
    - Preseason
        - Previous years' abundance
            - Age composition
        - Environmental excoriates (not currently used much)
    - During season
        -	Dam counts
        -	River flow
...
```{r}
library(tidyverse)
api_returns_forecasts |> filter(ReturnsForecastsStockName %in% c("Upriver Spring Chinook","Upper Columbia Summer Chinook","Upriver Bright (URB) Fall Chinook","Lower River Hatchery (LRH) Fall Chinook")) |> 
  filter(between(Year,2005,2023)) |> 
